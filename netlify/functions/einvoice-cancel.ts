...
export const handler: Handler = async (event) => {
  ...
  try {
    const { invoiceCode, remark } = JSON.parse(event.body || '{}')
    ...
    const payload: any = { timeStamp: ts, uncode, idno, sign, code: invoiceCode, remark: remark || '作廢' }
    const url = 'https://www.giveme.com.tw/invoice.do?action=cancelInvoice'
    const resp = await fetch(url, { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify(payload) })
    const text = await resp.text(); let data: any
    try { data = JSON.parse(text) } catch { data = { raw: text } }
    if (!resp.ok || data?.success === 'false') {
      return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: data?.msg || 'Cancel failed', data }) }
    }
