...
export const handler: Handler = async (event) => {
  ...
  try {
    const { orderId, buyer, items, amount, donation, phoneCarrier, orderCode, note } = JSON.parse(event.body || '{}')
    ...
    const totalFee = Math.max(1, Math.round(Number(amount || 0)))
    ...
    const payload: any = {
      ...
      phone: phoneCarrier || '',
      orderCode: orderCode || orderId || (buyer?.phone || ''),
      datetime,
      email: buyer?.email || '',
      state: donation ? '1' : '0',
      totalFee: String(totalFee),
      content: (note ? String(note) + ' ' : '') + (orderId ? `訂單:${orderId}` : ''),
      items: (Array.isArray(items) ? items : []).map((it: any) => ({
        name: String(it?.name || '品項'),
        money: Math.round(Number(it?.unitPrice || it?.money || 0)),
        number: Math.round(Number(it?.quantity || it?.number || 1))
      }))
    }

    const url = 'https://www.giveme.com.tw/invoice.do?action=addB2C'
    ...
    if (!resp.ok || data?.success === 'false') {
      return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: data?.msg || 'Create B2C failed', data }) }
    }
