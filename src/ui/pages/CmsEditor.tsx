    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            className="px-3 py-2 rounded bg-gray-800 text-white hover:bg-gray-700 disabled:opacity-60"
            onClick={() => handleToggleEnabled(!cmsEnabled)}
            disabled={toggling}
          >
            {cmsEnabled ? '關閉全站 CMS' : '開啟全站 CMS'}
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {content.services.map((s, idx) => (
            <div key={idx} className="border rounded p-4 space-y-3">
              ...
              <label className="block">
                <div className="text-sm text-gray-600 mb-1">連結（例：/store/products?category=cleaning）</div>
                <input
                  className="w-full border rounded px-3 py-2"
                  value={s.link ?? ''}
                  onChange={(e) => setService(idx, { link: e.target.value })}
                />
              </label>
              <label className="block">
                <div className="text-sm text-gray-600 mb-1">圖片 URL（選填）</div>
                <input
                  className="w-full border rounded px-3 py-2"
                  value={s.imageUrl ?? ''}
                  onChange={(e) => setService(idx, { imageUrl: e.target.value })}
                />
              </label>
            </div>
          ))}
