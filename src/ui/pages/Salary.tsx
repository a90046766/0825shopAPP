...
export default function SalaryPage() {
  ...
  useEffect(() => {
    (async()=>{
      try {
        ...
        const all = await a.payrollRepo.list(user)
  ...
  const calc = (r: PayrollRecord | null) => {
    ...
    const allowances = (r.allowances?.fuel||0) + (r.allowances?.overtime||0) + (r.allowances?.holiday||0) + (r.allowances?.duty||0)
    const deductions = (r.deductions?.leave||0) + (r.deductions?.tardiness||0) + (r.deductions?.complaints||0) + (r.deductions?.repairCost||0)
